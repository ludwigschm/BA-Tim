...
def test_pause_overlay_persists_across_multiple_toggles(monkeypatch):
    blocks = load_blocks()
    state = _make_practice_state(blocks)
    controller = TabletopController(state)
    monkeypatch.setattr(tabletop_view, "resolve_background_texture", lambda: None)
    ...
    for _ in range(3):
        ...
        view.pause_message = "Pause aktiv"
...
def test_pause_overlay_works_without_valid_ids_entry(monkeypatch):
    blocks = load_blocks()
    state = _make_practice_state(blocks)
    controller = TabletopController(state)
    monkeypatch.setattr(tabletop_view, "resolve_background_texture", lambda: None)
    ...
    # simulate weak/invalid ids access path by removing ids entry
    view.ids.pop("pause_cover", None)

    view.in_block_pause = True
    view.pause_message = "Pause via ObjectProperty"
    view.update_pause_overlay()
